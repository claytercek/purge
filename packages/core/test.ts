...
import { createPurge, type PurgeProvider, type PurgeClientConfig } from './mod.ts';

function createMockProvider() {
  const purgeCacheSpy = spy(() => okAsync(undefined));
  const getCacheHeadersSpy = spy(() => ({ 'X-Provider-Header': 'provider-value' }));
...
Deno.test("createPurge - cache control generation", async (t) => {
  const testCases = [
    {
      name: "private cache",
      params: { tags: ['tag1'], private: true, maxAgeCDN: 86400, maxAgeBrowser: 3600 },
